...
type Row = { id: string; description: string; location: string; months: string[] };

export default async function ViewMonthlyInspectionPage({
  params,
}: {
  ...
  try {
    if (inspection.data) {
      ...
      parsed = {
        columns: raw.columns ?? monthlyColumns,
    ...
    <div className="min-h-screen bg-white text-black">
      <div className="max-w-4xl mx-auto p-8">
        <div className="flex items-center justify-between gap-4 mb-8 pb-4 border-b-2 border-black">
          ...
          <a
            href={`/pdf-renderer?type=monthly-inspection&id=${encodeURIComponent(id)}`}
            target="_blank"
        ...
        <header className="border-b-2 border-black pb-4 mb-6">
          <h1 className="text-2xl font-bold text-black">Monthly Inspection Report</h1>
        </header>
        ...
        <footer className="mt-10 pt-4 border-t border-black text-center text-xs text-black">
          Safety System — Monthly Inspection Report
        </footer>
      </div>
    </div>
  );
