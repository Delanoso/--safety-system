...
export async function POST(req: Request) {
  ...
  const reason = data.reason != null ? String(data.reason).trim() || "Other" : "Other";
  const notes = data.notes != null ? String(data.notes).trim() || null : null;
  if (!Number.isInteger(itemTypeId) || !Number.isInteger(quantityDelta)) {
    return NextResponse.json(
      { error: "Valid itemTypeId and quantityDelta (integer) required." },
      { status: 400 }
    );
  }
  ...
  let newQty = currentQty + quantityDelta;
