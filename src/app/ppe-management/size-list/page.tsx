      ...
      <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="min-w-0">
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-black">PPE Size List</h1>
        ...
        <div className="rounded-2xl shadow-xl bg-white/60 backdrop-blur-xl border border-white/40 p-4 sm:p-6">
          <h2 className="text-base sm:text-lg font-bold text-black mb-4">Add department</h2>
        ...
        <div className="rounded-2xl shadow-xl bg-white/60 backdrop-blur-xl border border-white/40 overflow-hidden min-w-0">
          <h2 className="p-3 sm:p-4 bg-white/40 font-bold text-black border-b border-white/40 text-base sm:text-lg">Departments</h2>
          {departments.length === 0 ? (
            <p className="p-3 sm:p-4 text-black/60 text-sm sm:text-base">No departments yet. Add one above, then add sub-departments and people.</p>
          ) : (
            <ul className="divide-y divide-white/40">
              {departments.map((dept) => (
                <li key={dept.id} className="p-3 sm:p-4 flex items-center justify-between flex-wrap gap-2">
                  <div className="min-w-0">
                    <Link
                      href={`/ppe-management/size-list/department/${dept.id}`}
                      className="font-semibold text-black hover:underline text-base sm:text-lg"
                    >
                      {dept.name}
                    </Link>
